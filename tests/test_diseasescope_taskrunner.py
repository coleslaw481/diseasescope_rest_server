...
from diseasescope_rest_server.diseasescope_taskrunner import FileBasedTask
from diseasescope_rest_server.diseasescope_taskrunner import FileBasedSubmittedTaskFactory
from diseasescope_rest_server.diseasescope_taskrunner import DeletedFileBasedTaskFactory
...
class TestDiseasescopetaskrunner(unittest.TestCase):
    ...
    def test_save_task(self):
        ...
        try:
            ...
            self.assertEqual(task.save_task(), None)
            tfile = os.path.join(temp_dir, diseasescope_rest_server.TASK_JSON)
            ...
            self.assertEqual(task.save_task(), None)
            rfile = os.path.join(temp_dir, diseasescope_rest_server.RESULT)
    ...
    def test_move_task(self):
        temp_dir = tempfile.mkdtemp()
        try:
            submitdir = os.path.join(temp_dir, diseasescope_rest_server.SUBMITTED_STATUS)
            os.makedirs(submitdir, mode=0o755)
            processdir = os.path.join(temp_dir, diseasescope_rest_server.PROCESSING_STATUS)
            os.makedirs(processdir, mode=0o755)
            donedir = os.path.join(temp_dir, diseasescope_rest_server.DONE_STATUS)
            os.makedirs(donedir, mode=0o755)

            # try a move on unset task
            task = FileBasedTask(None, None)
            self.assertEqual(task.move_task(diseasescope_rest_server.PROCESSING_STATUS),
            ...
            self.assertEqual(task.move_task(diseasescope_rest_server.SUBMITTED_STATUS),
                             None)
            ...
            self.assertTrue(diseasescope_rest_server.PROCESSING_STATUS in
                            task.get_taskdir())

            # try a move from process to done
            self.assertEqual(task.move_task(diseasescope_rest_server.DONE_STATUS),
                             None)
            self.assertTrue(diseasescope_rest_server.DONE_STATUS in
                            task.get_taskdir())

            # try a move from done to submitted
            self.assertEqual(task.move_task(diseasescope_rest_server.SUBMITTED_STATUS),
                             None)
            self.assertTrue(diseasescope_rest_server.SUBMITTED_STATUS in
                            task.get_taskdir())

            # try a move from submitted to error
            self.assertEqual(task.move_task(diseasescope_rest_server.ERROR_STATUS),
                             None)
            self.assertTrue(diseasescope_rest_server.DONE_STATUS in
                            task.get_taskdir())
            tjson = os.path.join(task.get_taskdir(), diseasescope_rest_server.TASK_JSON)
            with open(tjson, 'r') as f:
                data = json.load(f)
                self.assertEqual(data[diseasescope_rest_server.ERROR_PARAM],
                                 'Unknown error')
    ...
    def test_filebasedsubmittedtaskfactory_get_next_task(self):
        ...
        try:
            ...
            # empty submit dir
            sdir = os.path.join(temp_dir, diseasescope_rest_server.SUBMITTED_STATUS)
            ...
            # empty json file
            taskjsonfile = os.path.join(taskdir, diseasescope_rest_server.TASK_JSON)
            ...
            goodjson = os.path.join(anothertask, diseasescope_rest_server.TASK_JSON)
    ...
    def test_deletefilebasedtaskfactory_get_task_with_id(self):
        ...
        try:
            ...
            # a directory
            submitfile = os.path.join(temp_dir, diseasescope_rest_server.SUBMITTED_STATUS,
                                      '1.2.3.4', 'foo')
            ...
            procdir = os.path.join(temp_dir, diseasescope_rest_server.PROCESSING_STATUS,
                                   '4.5.5.5',
    ...
    def test_deletefilebasedtaskfactory_get_next_task(self):
        ...
        try:
            ...
            del_req_dir = os.path.join(temp_dir, diseasescope_rest_server.DELETE_REQUESTS)
            ...
            with open(a_request, 'w') as f:
                f.write('1.2.3.4')
                f.flush()
            done_dir = os.path.join(temp_dir, diseasescope_rest_server.DONE_STATUS,
                                    '1.2.3.4',
                                    '02e487ef-79df-4d99-8f22-1ff1d6d52a2a')
